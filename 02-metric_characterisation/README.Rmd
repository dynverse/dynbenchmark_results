...
csl: ../../manuscript/nature-biotechnology.csl
link-citations: true
---
...
```{r, results = "asis"}
add_fig(
  result_file("clustering_scores_overview.rds", experiment = "02-metric_characterisation/01-individual_metrics"),
...
When the position of a cell is the same in both the gold standard and the prediction, its _relative_ distances to all other cells in the trajectory should also be the same. This observation is the basis for the `r label_metric("correlation")` metric.
...
The geodesic distance is the distance a cell has to go through the trajectory space to get from one position to another (`r ref("fig", "metrics_geodesic")`). The way this distance is calculated depends on how two cells are positioned:
...
```{r, results = "asis"}
add_fig(
  result_file("waypoints_overview.rds", experiment = "02-metric_characterisation/01-individual_metrics"),
  "waypoints_overview",
...
```{r, results = "asis"}
add_fig(
  result_file("featureimp_cor_distributions.rds", experiment = "02-metric_characterisation/01-individual_metrics"),
...
- Its **topology**, using the `r label_metric("edge_flip")`
...
walkdf(assessments, function(assessment) {
  cat(
    knitr::knit_child(result_file("assessment.Rmd", experiment = "02-metric_characterisation/02-metric_conformity"), quiet=TRUE, envir = environment())
  )
})
```
